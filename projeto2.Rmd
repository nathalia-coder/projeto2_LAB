...
df_reg <- df_completo %>% 
  select(
    grupo,
    # numericas
    razaoldl, ldl_dosado, pressao_media, pressao_dias, pressao_sis, creatinina, trombomodulina,
    imc, adma, lym, filtr_glomerular, rbc, wbc, hdl, triglicerides, colesterol_total,
    # categoricas
    hipertensao, gpsanguineo, escolaridade_bin, categoriaimc, imcbinario
    )
```
...
ConcluiAjuste <- function(fit){
  tabela <- cbind(
    Variavel = names(coef(fit))[-1],
    Efeito = paste0(100 * (exp( coef(fit)[-1]) - 1) %>% round(4),"%")
    ) %>% as_tibble()
...
## Para Pressão Arterial Sistólica (PAS)

Categorização: 

*Normal:* Pressão Sistólica < 140mmHg
*Alterada:* Pressão Sistólica > 140mmHg

```{r resposta_reg_pressao_sis}
df_reg <- df_reg %>% mutate(
  pas = ifelse(pressao_sis < 140, 0, 1) %>% as.factor()
)
```

* IMC não categorizado

```{r ajuste_pressao_sis1}
# Modelo inicial
teste <- glm(
  formula = pas ~ grupo + razaoldl + creatinina + imc + hdl + hipertensao + gpsanguineo + escolaridade_bin,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()

# Modelo final
teste <- glm(
  formula = pas ~ grupo + hipertensao + imc,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()
ConcluiAjuste(teste)
```

* IMC categorizado nenhum deu significativo

* Usando Tromobulina e Adma obtivemos algo não significativo para ambos

```{r ajuste_pressao_sis2}
teste <- glm(
  formula = pas ~ grupo + hipertensao + imc + adma + trombomodulina,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc, adma, trombomodulina))
teste %>% summary()
ConcluiAjuste(teste)
```

## Para Pressão Arterial Diástólica (PAD)

Categorização: 

*Normal:* Pressão Diástólica < 90mmHg
*Alterada:* Pressão Diástólica > 90mmHg

```{r resposta_reg_pressao_dias}
df_reg <- df_reg %>% mutate(
  pad = ifelse(pressao_dias < 90, 0, 1) %>% as.factor()
)
```

* IMC não categorizado

Ou razão ldl colesterol era significativo ou ldl_dosado. Só *LDL Dosado* foi significativo no modelo final. Razão ldl deu grande "efeito" mas com alta variabilida, tornando-o não significativo.


```{r ajuste_pressao_dias1}
# Modelo inicial
teste <- glm(
  formula = pad ~ grupo + razaoldl + creatinina + imc + hdl + hipertensao + gpsanguineo + escolaridade_bin,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()

# Modelo final
teste <- glm(
  formula = pad ~ grupo + hipertensao + ldl_dosado + imc,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()
ConcluiAjuste(teste)
```
...
A categoria binária resultou em algo contraintuitivo, que estar com imc alterado resultava em redução na chance de estar com alta pressão diastólica

```{r}
teste <- glm(
  formula = pad ~ grupo + hipertensao + ldl_dosado + imcbinario,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()
ConcluiAjuste(teste)

teste <- glm(
  formula = pad ~ grupo + hipertensao + ldl_dosado + categoriaimc,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc))
teste %>% summary()
ConcluiAjuste(teste)
```
...
```{r ajuste_pressao_dias2}
teste <- glm(
  formula = pad ~ grupo + hipertensao + imc + adma + trombomodulina,
  family = binomial(link = "logit"), data = df_reg %>% drop_na(imc, adma, trombomodulina))
teste %>% summary()
ConcluiAjuste(teste)
```
...
## Para Pressão Arterial Sistólica

```{r}
teste <- lm(pressao_sis ~ grupo + hipertensao + imc , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
...
```{r}
teste <- lm(pressao_sis ~ grupo + hipertensao + categoriaimc , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
```{r}
teste <- lm(pressao_sis ~ grupo + hipertensao + imcbinario , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```

## Para Pressão Arterial Diástólica

```{r}
teste <- lm(pressao_dias ~ grupo + hipertensao + imc , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
```{r}
teste <- lm(pressao_dias ~ grupo + hipertensao + categoriaimc , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
```{r}
teste <- lm(pressao_dias ~ grupo + hipertensao + imcbinario , data = df_reg)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```

## Para Pressão Arterial Média

Foi retirado um outlier

* IMC não categorizado

```{r}
df_reg_sem_out <- df_reg[-172,]

teste <- lm(pressao_media ~ grupo + hipertensao + imc , data = df_reg_sem_out)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
```{r}
teste <- lm(pressao_media ~ grupo + hipertensao + categoriaimc , data = df_reg_sem_out)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
```{r}
teste <- lm(pressao_media ~ grupo + hipertensao + imcbinario , data = df_reg_sem_out)
teste %>% summary()
reglin::ggresiduals(teste)
shapiro.test(teste$residuals)
```
...
resultados_p_valores <- data.frame(
  Variavel = binarias, 
  P_valor_IMC = p_valores_imc, 
  P_valor_Pressao_Media = p_valores_pressao_media, 
  P_valor_Colesterol_Total = p_valores_colesterol_total)

# Visualizar os resultados
resultados_p_valores %>% 
  kbl(
    digits = 3,
    format.args = list(decimal.mark = ',', big.mark = ".") 
    ) %>%
...
for (numerica in numericas) {
  ...
  resultados_kw[[numerica]] <- data.frame(Categoria = categorias, P_Valor = p_valores %>% round(3))
}

# Exibir os resultados - original
# for (numerica in numericas) {
#   cat(sprintf("\n=== Resultados para %s ===\n", numerica))
...
```{r}
# Exibir os resultados - para html
resultados_kw[[1]] %>% 
  kbl(
    format.args = list(decimal.mark = ',', big.mark = ".") 
    ) %>%
...
**Resultado para colesterol_total:**
