...
```{r}
modelopas2 <- lm(pressao_sis ~ grupo + idade + anti_hiper + imc + escolaridade, data = df_reg)
modelopas2 %>% summary()
...
```{r}
modelopad2 <- lm(pressao_dias ~ grupo + anti_hiper + imc + idade + escolaridade, data = df_reg)
modelopad2 %>% summary()
...
-   **M2**: Modelo com escolaridade sendo significativa
```{r}
modelopam2 <- lm(pressao_media ~ grupo + anti_hiper + imc + idade + escolaridade, data = df_reg)
modelopam2 %>% summary()
...
## Anti Hipercolesterolemia (não faz sentido colocar colesterol total, faz? Anti hipertensivo muito estranho na tabela)

```{r}
modeloantihipercol <- glm(anti_hipercolesterolemia ~ anti_hiper + idade, data = df_reg, family = "binomial"(link = 'logit'))
summary(modeloantihipercol)
#vif(modeloantihipercol)
hnp(modeloantihipercol)
tbl_regression(modeloantihipercol, exponentiate = TRUE)
ConcluiAjuste(modeloantihipercol)
```

## Remédio Diabetes (nada deu significativo)

```{r}
modelodiabetes<- glm(anti_diabetes ~ imc + anti_hiper, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelodiabetes)
#vif(modelodiabetes)
hnp(modelodiabetes)
tbl_regression(modelodiabetes, exponentiate = TRUE)
ConcluiAjuste(modelodiabetes)
```

## PA > 130/85 e/ou uso de anti-hipertensivo

```{r}
modelopa <- glm(paalto ~ grupo + idade + imc, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelopa)
vif(modelopa)
#hnp(modelopa)
tbl_regression(modelopa, exponentiate = TRUE)
ConcluiAjuste(modelopa)
```

## Cintura \> 88cm (quando entra o grupo não é significativo, mas é 0,09)

```{r}
modelocintura<- glm(cinturaaumentada ~ pressao_media + triglicerides, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelocintura)
#vif(modelocintura)
hnp(modelocintura)
tbl_regression(modelocintura, exponentiate = TRUE)
ConcluiAjuste(modelocintura)
```

## TG \> 150 mg/dL (com creatinina)

```{r}
modelotg <- glm(tgalto ~ grupo + creatinina + imc + colesterol_total, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelotg)
#vif(modelotg)
hnp(modelotg)
tbl_regression(modelotg, exponentiate = TRUE)
ConcluiAjuste(modelotg)
```

## TG \> 150 mg/dL (sem creatinina)

* Grupo deu não significativo

```{r}
modelotg <- glm(tgalto ~ imc + colesterol_total, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelotg)
#vif(modelotg)
hnp(modelotg)
tbl_regression(modelotg, exponentiate = TRUE)
...
```{r}
modelohdl <- glm(hdlbaixo ~ imc + triglicerides, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelohdl)
#vif(modelohdl)
hnp(modelohdl)
tbl_regression(modelohdl, exponentiate = TRUE)
ConcluiAjuste(modelohdl)
```
