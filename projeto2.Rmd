...
qqnorm(modeloimc$residuals); qqline(modeloimc$residuals)
ConcluiAjuste(modeloimc)
#reglin::ggresiduals(fit_final)
```

### LDL pq deixamos triglicerides?

```{r}
modeloldl <- lm(ldl_dosado ~ grupo + idade + triglicerides, df_num)
summary(modeloldl)
...
```{r}
modeloadma <- lm(adma ~ idade + ldl_dosado, df_reg)
summary(modeloadma)
shapiro.test(modeloldl$residuals)
qqnorm(modeloldl$residuals); qqline(modeloldl$residuals)
#reglin::ggresiduals(fit_final)
...
```{r echo=FALSE}
modelotg <- glm(tgalto ~ creatinina + imc + colesterol_total, data = df_reg, family = "binomial"(link = 'logit'))
summary(modelotg)
car::vif(modelotg)
hnp(modelotg)
