...
```
No modelo para pressão sistólica, as variáveis grupo, idade, uso de anti-hipertensivo e IMC foram consideradas significativas, explicando 24% da variabilidade (\(R^2\)=24%).

Observando a segunda coluna, notamos que o coeficiente para o grupo1 é 6,44. Portanto, mantendo todas as outras variáveis constantes, pertencer ao grupo1 está associado a um acréscimo de 6,44 unidades na pressão arterial em relação ao grupo de referência. Esse aumento é observado para cada unidade de mudança na variável de interesse associada ao grupo1.

De maneira análoga, podemos perceber que o acréscimo de um ano de idade implica um aumento de 0,51 mmHg na pressão arterial sistólica (PAS). O uso de anti-hipertensivos está associado a um aumento de 10,57 mmHg na PAS, enquanto o aumento em uma unidade de IMC resulta em um acréscimo de 1,08 mmHg na PAS.

Esses resultados indicam as relações específicas entre as variáveis do modelo e a pressão sistólica, fornecendo insights importantes sobre os fatores que influenciam os níveis de pressão arterial.
...
```
No novo modelo, ao incluir a variável de escolaridade, observamos mudanças nos impactos das variáveis previamente consideradas.

Mantendo as demais variáveis constantes, pertencer ao grupo1 está associado a um aumento de 5,8007 mmHg na pressão arterial em comparação com o grupo de referência. O aumento em um ano de idade agora implica um acréscimo de 0,4674 mmHg na pressão arterial sistólica (PAS). Tomar anti-hipertensivo continua associado a um aumento significativo de 9,6290 mmHg na PAS.
...
```
No modelo para pressão sistólica, as variáveis grupo, uso de anti-hipertensivo, idade e IMC foram consideradas significativas, explicando 24% da variabilidade (\(R^2\)=24%).

Na análise de regressão para a pressão arterial diastólica (PAD), as mudanças nos coeficientes revelam associações distintas com as variáveis consideradas. Mantendo as outras variáveis constantes, pertencer ao grupo1 está associado a um aumento de 5,64mmHg na pressão arterial diastólica em comparação com o grupo de referência. 

O uso de anti-hipertensivo está significativamente associado a um aumento de 8,44mmHg na PAD. Além disso, um acréscimo de uma unidade no índice de massa corporal (IMC) resulta em um aumento de 0,73mmHg na PAD. A idade também desempenha um papel, onde um aumento de um ano está associado a um acréscimo de 0,39mmHg na PAD. 
...
No modelo para pressão sistólica, as variáveis grupo, uso de anti-hipertensivo, idade e IMC foram consideradas significativas, explicando 26% da variabilidade (\(R^2\)=26%).

Os resultados indicam as seguintes associações: mantendo as demais variáveis constantes, pertencer ao grupo1 está associado a um aumento de 5,9136 unidades na pressão arterial média em comparação com o grupo de referência. O uso de anti-hipertensivo está significativamente associado a um aumento de 9,1520 unidades na pressão arterial média. Além disso, um acréscimo de uma unidade no índice de massa corporal (IMC) resulta em um aumento de 0,8516 unidades na pressão arterial média. A idade também desempenha um papel, onde um aumento de um ano está associado a um acréscimo de 0,4336 unidades na pressão arterial média.
