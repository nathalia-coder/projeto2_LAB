...
cat <- c("grupo", "etnia", "escolaridade", "doenca", "num_gestacoes", "gpsanguineo",
         "rh", "medic_diabetes", "medic_hipercol", "tabagista", "elitista")
num <- c("grupo", cols[! cols %in% cat])

df_num <- df_completo %>% select(all_of(num))
df_cat <- df_completo %>% select(all_of(cat))

df_cat <- df_cat %>% mutate(escolaridade_bin = ifelse(escolaridade %in% c(3, 4), "superior", "basico"))
```
...
realizar_teste <- function(dataframe, variavel) {
  ...
  if (shapiro1$p.value <= alpha || shapiro2$p.value <= alpha) { # pelo menos um nao normal
...
```{r}
tabela_resumo_sig <- tabela_resumo %>% filter(Conc == "Sig") %>% select(- Conc)
...
```{r}
tabela_resumo_cat_sig <- tabela_resumo_cat %>% filter(Conc == "Sig") %>% select(- Conc)

kbl(
  tabela_resumo_cat_sig,
  digits = 2,
  format.args = list(decimal.mark = ',', big.mark = ".")
  ) %>%
  kable_paper(full_width = F) %>%
