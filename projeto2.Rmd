...
Os resultados foram adicionados em uma tabela resumo, que contém, além da variável analisada e o grupo MN ou MP, as seguintes medidas descritivas, para os dados contínuos:
...
Foi feito o *Teste de Shapiro-Wilk* para avaliar se era válida a suposição de normalidade para os grupos. Foi aplicado o *Teste T* para comparação da média de dois grupos, assim como o *Teste de Mann-Whitney*, que foi aplicado como teste não paramétrico. Em suma, deseja-se avaliar se a média ea mediana entre os grupos é igual para uma dada variável ou se há diferença. Apesar de algumas veriáveis não terem atendido ao pressuposto de normalidade, os dois testes foram feitos para todas as variáveis, ficando a critério da pesqwuisadora uma possível padronização.
...
As variáveis explicativas pertencentes a cada um dos modelos finais foram apenas aquelas consideradas estatisticamente e clinicamente significativas em relação ao que estava sendo estudado.

Os resíduos para a maioria dos modelos seguiu a normalidade, e os pressupostos para realizar a regressão foram seguidos (o que não deu normalidade foi )
